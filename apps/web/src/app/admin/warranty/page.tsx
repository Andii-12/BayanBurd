...
export default function AdminWarranty() {
  const { data } = useQuery({ queryKey: ["assets-all"], queryFn: () => api("/api/assets?limit=100") });
  return (
    <div>
      <h1 className="text-xl font-semibold">Баталгаа</h1>
      <div className="card mt-4 overflow-x-auto p-0">
        <table className="w-full text-sm">
          ...
          <tbody>
            {(data?.items || []).filter((a: any) => a.warrantyEndDate).map((a: any) => (
              <tr key={a._id} className="border-t border-[#E5E7EB]">
                ...
                <td>{daysRemaining(a.warrantyEndDate)}</td>
              </tr>
            ))}
          </tbody>
        </table>
